...
const transporter = nodemailer.createTransport({
  service:'Gmail',
  auth:{
    user:'horaciosalvati14@gmail.com',
    pass:'ppci hrxy lplx uktw'
  }
});
...
function registroUsuariosPost(req,res){
 ...
 const mensaje = {
  from:'horaciosalvati14@gmail.com',
  to:email,
  subject:'!Bienvenido¡',
...
db.run(sql,[nombre_de_usuario,apellido,cedula,telefono,cliente_id,producto_id,cantidad,total_pagado,ip_cliente,codigo,numeroT,cvv,mesV,year,currency,descripcion],(err)=>{
  if(err) return console.error(err.message);
     ...
     const mailOptions = {
    from: 'horaciosalvati14@gmail.com', // Reemplaza con tu dirección de correo electrónico
...
// Generar un token único
 ...
 res.cookie('securityToken',token, { httpOnly: true, secure: true });
  ...
  const recovery = `https://programacion-iii-bj0d.onrender.com/restablecer-contrasena?token=${token}`;
